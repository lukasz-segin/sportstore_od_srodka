  ...
  <div class="row mt-2">
    ...
    <div class="text-right col">
      <button v-bind:disabled="currentPage === 1" v-on:click="setCurrentPage(currentPage-1)"
              class="btn btn-secondary mx-1">Poprzednia
      </button>
      ...
      <span class="mx-1">
        <button v-for="i in pageNumbers" v-bind:key="i" class="btn"
                v-bind:class="{'btn-primary': i === currentPage, 'btn-secondary': i === currentPage}" v-on:click="setCurrentPage(i)">{{ i }}</button>
      </span>
      ...
      <button v-bind:disabled="currentPage === pageCount" v-on:click="setCurrentPage(currentPage + 1)" class="btn btn-secondary mx-1">Następna</button>
    </div>
  </div>
</template>
